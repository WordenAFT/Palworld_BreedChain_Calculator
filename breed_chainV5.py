...
def calculate_offspring(pal_1, pal_2, excluded_pals):
    ...
    file_order = ['Lamball', 'Cattiva', 'Chikipi', 'Lifmunk', 'Foxparks', 'Fuack',
       'Sparkit', 'Tanzee', 'Rooby', 'Pengullet', 'Penking', 'Jolthog',
       'Jolthog Cryst', 'Gumoss', 'Gumoss (Special)', 'Vixy', 'Hoocrates',
       'Teafant', 'Depresso', 'Cremis', 'Daedream', 'Rushoar', 'Nox',
       'Fuddler', 'Killamari', 'Mau', 'Mau Cryst', 'Celeray', 'Direhowl',
       'Tocotoco', 'Flopie', 'Mozzarina', 'Bristla', 'Gobfin', 'Gobfin Ignus',
       'Hangyu', 'Hangyu Cryst', 'Mossanda', 'Mossanda Lux', 'Woolipop',
       'Caprity', 'Melpaca', 'Eikthyrdeer', 'Eikthyrdeer Terra', 'Nitewing',
       'Ribunny', 'Incineram', 'Incineram Noct', 'Cinnamoth', 'Arsox', 
       'Dumud', 'Cawgnito', 'Leezpunk', 'Leezpunk Ignis', 'Loupmoon',
       'Galeclaw', 'Robinquill', 'Robinquill Terra', 'Gorirat', 'Beegarde',
       'Elizabee', 'Grintale', 'Swee', 'Sweepa', 'Chillet', 'Univolt',
       'Foxcicle', 'Pyrin', 'Pyrin Noct', 'Reindrix', 'Rayhound', 'Kitsun',
       'Dazzi', 'Lunaris', 'Dinossom', 'Dinossom Lux', 'Surfent',
       'Surfent Terra', 'Maraith', 'Digtoise', 'Tombat', 'Lovander',
       'Flambelle', 'Vanwyrm', 'Vanwyrm Cryst', 'Bushi', 'Beakon', 'Ragnahawk',
       'Katress', 'Wixen', 'Verdash', 'Vaelet', 'Sibelyx', 'Elphidran',
       'Elphidran Aqua', 'Kelpsea', 'Kelpsea Ignus', 'Azurobe', 'Cryonlinx',
       'Blazehowl', 'Blazehowl Noct', 'Relaxaurus', 'Relaxaurus Lux',
       'Broncherry', 'Broncherry Aqua', 'Petallia', 'Reptyro', 'Reptyro Cryst',
       'Kingpaca', 'Kingpaca Cryst', 'Mammorest', 'Mammorest Cryst', 'Wumpo',
       'Wumpo Botan', 'Warsect', 'Fenglope', 'Felbat', 'Quivern', 'Blazamut',
       'Helzephyr', 'Astegon', 'Menasting', 'Anubis', 'Jormuntide',
       'Jormuntide Ignis', 'Suzaku', 'Suzaku Aqua', 'Grizzbolt', 'Lyleen',
       'Lyleen Noct', 'Faleris', 'Orserk', 'Shadowbeak', 'Paladius',
       'Necromus', 'Frostallion', 'Frostallion Noct', 'Jetragon']
    
    file_index_map = {name.lower(): index for index, name in enumerate(file_order, start=1)}
    ...
    special_combinations = 'Relaxaurus Lux, Incineram Noct, Mau Cryst, Vanwyrm Cryst, Eikthyrdeer Terra, Elphidran Aqua, Pyrin Noct, Mammorest Cryst, Mossanda Lux, Dinossom Lux, Jolthog Cryst, Frostallion Noct, Kingpaca Cryst, Ice Kingpaca, Lyleen Noct, Leezpunk Ignis, Blazehowl Noct, Robinquill Terra, Broncherry Aqua, Surfent Terra, Gobfin Ignus, Suzaku Aqua, Reptyro Cryst, Hangyu Cryst, Lyleen, Faleris, Grizzbolt, Orserk, Shadowbeak'
    ...
    # Calculate the raw power
    offspring_power_raw = math.floor((breed_power_dict[pal_1] + breed_power_dict[pal_2] + 1) / 2)

    # Sort the breed powers and their names
    sorted_powers = sorted(breed_power_dict.items(), key=lambda item: item[1])
    closest_lower = None
    closest_higher = None

    # Iterate to find the closest lower and higher values
    for name, power in sorted_powers:
        if name not in special_combinations_list:
            if power <= offspring_power_raw:
                closest_lower = (name, power)
            elif power > offspring_power_raw and closest_higher is None:
                closest_higher = (name, power)

    # Check if closest lower or higher are found
    if not closest_lower or not closest_higher:
        return None, None  # No valid pairing found

    closest_lower_name, closest_lower_power = closest_lower
    closest_higher_name, closest_higher_power = closest_higher

    # Calculate differences
    diff1 = abs(offspring_power_raw - closest_lower_power)
    diff2 = abs(offspring_power_raw - closest_higher_power)

    # Determine closest name based on the differences and file index order
    if diff1 == diff2:
        if file_index_map.get(closest_lower_name) > file_index_map.get(closest_higher_name):
            closest_name = closest_higher_name
        else:
            closest_name = closest_lower_name
    elif diff1 < diff2:
        closest_name = closest_lower_name
    else:
        closest_name = closest_higher_name
...
def find_key_of_closest_value(target_value, dictionary, excluded_pals):
    ...
    for key, value in dictionary.items():
        ...
        if current_difference < smallest_difference:
            ...
            closest_key = key
        # If there's a tie, choose the larger value (hence the closer key)
