...
def calculate_breed_chain(target_pal, inherited_pals, excluded_pals):  
    ...
    # special_combinations = ['Relaxaurus Lux', 'Incineram Noct', 'Mau Cryst',
    ...
    special_combinations_list = special_combinations.split(', ')
    ...
    try:
        ...
        if target_pal not in breed_power_dict:
            print(f"Error: Target pal '{target_pal}' not found. Type better idiot.")
            return f"Error: Target pal '{target_pal}' not found."
        
        if len(inherited_pals) > 1:
            for pal in inherited_pals:
                if pal not in breed_power_dict:
                    print(f"Error: Inherited pal '{pal}' not found. Type better idiot.")
                    ...
                    return f"Error: Inherited pal '{pal}' not found."
        else: 
            if inherited_pals[0] not in breed_power_dict:
                print("Error: Inherited pal '{pal}' not found. Type better idiot.")
                print(f'\nBut also... If selecting multiple inherited pals, don\'t place a space after each comma. For two species, for example, youd write: Jetragon,Bristla')
                return f"Error: Inherited pal '{pal}' not found."
        ...
        if len(inherited_pals) >= 1:      
            for inherited_pal in inherited_pals[1:]:
                ...
                print(f'Bred {old_pal} with {inherited_pal} to get {current_pal} with power {current_power}')
            else: 
        ...
        while offspring_name != target_pal:
            ...
            if power_req >= 1500:
                ...
                print(f'Step {step}: Breed: {old_offspring} + {highest_avail_pal} = {offspring_name}; Offspring Power: {offspring_power}')
                ...
                print(f'Step {step}: Breed: {old_offspring} + {lowest_avail_pal} = {offspring_name}; Offspring Power: {offspring_power}')
            else: 
                ...
                if offspring_name != target_pal:
                    print(f"\nTwo Step Lookahead Detected Infinite Loop on Breed with: {offspring_name} & {next_breed}. Randomizing Breeding Target...\n")
                    #Shuffle the breeding by mixing the offspring with a random pal within 200 points:
                    next_breed = select_random_key_within_range(breed_power_dict, breed_power_dict[offspring_name], 200, excluded_pals)

                    offspring_name, offspring_power = calculate_offspring(old_offspring, next_breed, excluded_pals)

            
                print(f'Step {step}: Breed: {old_offspring} + {next_breed} = {offspring_name}; Offspring Power: {offspring_power}')
...
if __name__ == "__main__":
    target_pal = 'Foxparks'
    inherited_pals = ['Jetragon']
    excluded_pals = ['Chikipi']
